...
class HousePrices(scrapy.Spider):
    name = 'alonhadat'
    start_urls = [
        f'https://alonhadat.com.vn/nha-dat/cho-thue/nha-dat/1/ha-noi/trang--{e}.html' for e in range(1, 1342)
    ]

    def parse(self, response):
        # print(response.css('.content-items::text').extract())
        for quote in response.css('#left .content-items .content-item'):
            # print(quote.css('::text').extract())
            main_link = quote.css('.thumbnail a::attr("href")').get()
